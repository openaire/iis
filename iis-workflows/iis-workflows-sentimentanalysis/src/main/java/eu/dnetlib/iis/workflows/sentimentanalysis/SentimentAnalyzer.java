 ...
 * @author mhorst
 */
public class SentimentAnalyzer extends
        Mapper<AvroKey<DocumentTextWithReferences>, NullWritable, AvroKey<ReferenceWithSentimentLabels>, NullWritable> {
    ...
    protected void map(AvroKey<DocumentTextWithReferences> key, NullWritable value, Context context)
        ...
        try {
            ...
            List<ReferenceWithSentimentLabels> results = convertCitationSentiment(input.getId(), input.getReferences(),
                    extractor.getCitationSentiments());
    ...
    /**
     * Converts citation sentiments into {@link ReferenceWithSentimentLabels}
     * objects.
     * 
     * @param publicationId
     * @param references
     * @param sentiments
     * @return citation sentiments converted into
     *         {@link ReferenceWithSentimentLabels} objects
     */
    protected static List<ReferenceWithSentimentLabels> convertCitationSentiment(CharSequence publicationId,
        ...
        Preconditions.checkArgument(references.size() != sentiments.size(), "number of sentiments: %s is not equal the number of input references: %s", 
                sentiments.size(), references.size());
        List<ReferenceWithSentimentLabels> results = new ArrayList<ReferenceWithSentimentLabels>(references.size());
        for (int i = 0; i < references.size(); i++) {
            ReferenceWithSentimentLabels result = convertCitationSentiment(publicationId, references.get(i),
                    sentiments.get(i));
            if (result != null) {
                results.add(result);
            }
     ...
     * @param reference
     * @param sentiment
     * @return citation sentiment converted to
     *         {@link ReferenceWithSentimentLabels} or null when no sentiment
     *         generated
     */
    protected static ReferenceWithSentimentLabels convertCitationSentiment(CharSequence publicationId,
                ...
                if (basicMetadata.getPages() != null) {
                    if (basicMetadata.getPages().getStart() != null
                        ...
                        if (basicMetadata.getPages().getEnd() != null
                            ...
                            // range
                            StringBuilder pagesField = new StringBuilder(basicMetadata.getPages().getStart());
                            pagesField.append('-');
                            pagesField.append(basicMetadata.getPages().getEnd());
                            bibEntry.addField(BibEntry.FIELD_PAGES, pagesField.toString());
                        } else {
                            // start only
                            bibEntry.addField(BibEntry.FIELD_PAGES, basicMetadata.getPages().getStart().toString());
                        }
                    }
                }
                if (basicMetadata.getSource() != null) {
                    bibEntry.addField(BibEntry.FIELD_JOURNAL, basicMetadata.getSource().toString());
                }
                if (basicMetadata.getTitle() != null) {
                    bibEntry.addField(BibEntry.FIELD_TITLE, basicMetadata.getTitle().toString());
                }
                if (basicMetadata.getVolume() != null) {
                    bibEntry.addField(BibEntry.FIELD_VOLUME, basicMetadata.getVolume().toString());
                }
                if (basicMetadata.getYear() != null) {
                    bibEntry.addField(BibEntry.FIELD_YEAR, basicMetadata.getYear().toString());
                }
                if (basicMetadata.getEdition() != null) {
                    bibEntry.addField(BibEntry.FIELD_EDITION, basicMetadata.getEdition().toString());
                }
                if (basicMetadata.getPublisher() != null) {
                    bibEntry.addField(BibEntry.FIELD_PUBLISHER, basicMetadata.getPublisher().toString());
                }
                if (basicMetadata.getLocation() != null) {
                    bibEntry.addField(BibEntry.FIELD_LOCATION, basicMetadata.getLocation().toString());
                }
                if (basicMetadata.getSeries() != null) {
                    bibEntry.addField(BibEntry.FIELD_SERIES, basicMetadata.getSeries().toString());
                }
                if (basicMetadata.getIssue() != null) {
                    bibEntry.addField(BibEntry.FIELD_NUMBER, basicMetadata.getIssue().toString());
                }
                if (basicMetadata.getUrl() != null) {
                    bibEntry.addField(BibEntry.FIELD_URL, basicMetadata.getUrl().toString());
                }
            }
