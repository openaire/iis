...
import eu.dnetlib.iis.common.avro.personwithdocuments.Document;
...
public class SparkPipeExecutorTest {
	...
	public void mapTest() throws Exception {
		...
		// assert
		assertSame(mappedKeyValueRDD, retMappedRDD);
	...
	public void reduceTest() throws Exception {
		...
		// execute
		JavaPairRDD<AvroKey<GenericRecord>, NullWritable> retReducedRDD = pipeExecutor
				.doReduce(mappedKeyValueRDD, "reducerScriptName", "-arg=value", Document.class);

		// assert
		assertSame(reducedAvroKeyValueRDD, retReducedRDD);
	...
	private void assertKeyValueToStringFunction(Function<Tuple2<String, String>, String> function) throws Exception {
		assertEquals("key\tvalue", function.call(new Tuple2<>("key", "value")));
		assertEquals("key_only", function.call(new Tuple2<>("key_only", null)));
	}
