...
public class AffMatchingDedupJob {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (JavaSparkContext sc = JavaSparkContextFactory.withConfAndKryo(new SparkConf())) {
            ...
            JavaPairRDD<String, Tuple2<MatchedOrganization,String>> inputAPair = inputA
                    .mapToPair(x -> new Tuple2<>(generateKey(x), new Tuple2<>(x, inferenceProvenanceInputA)));
            
            JavaPairRDD<String, Tuple2<MatchedOrganization,String>> inputBPair = inputB
                    .mapToPair(x -> new Tuple2<>(generateKey(x), new Tuple2<>(x, inferenceProvenanceInputB)));
            
            JavaPairRDD<String, Tuple2<MatchedOrganization,String>> unionedPair = inputAPair.union(inputBPair);
            
            JavaRDD<MatchedOrganizationWithProvenance> results = unionedPair
                    .reduceByKey((x, y) -> x._1.getMatchStrength() >= y._1.getMatchStrength() ? x : y)
                    .map(x -> buildMatchedOrganizationWithProvenance(x._2));
    ...
    private static String generateKey(MatchedOrganization source) {
        return String.valueOf(source.getDocumentId()) + '|' + source.getOrganizationId();
    }
