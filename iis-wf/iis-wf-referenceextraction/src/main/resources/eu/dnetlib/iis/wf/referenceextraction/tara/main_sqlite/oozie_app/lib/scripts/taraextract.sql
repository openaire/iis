...
( setschema 'c1,id,matches' select c1, id, regexpcountwords('\b'||tarakeywords||'\b', c2) as matches from (
...
( setschema 'c1,id,matches,prev,middle,next' select c1, id, regexpcountwords('\b'||tarakeywords||'\b', c2) as matches, textwindow2s(keywords(c2), 4,1,4, '(\b(?:tara)\b)') from (
