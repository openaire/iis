...
select jdict('documentId', docid, 'datasetId', dsetID, 'confidenceLevel', sqroot(min(1.49,max(confidence))/1.5),'textsnippet',context) from
(
select docid, dsetID, case when length(context) > 120 then round((length(titles) + conf*10)/(length(context)*1.0),2) else round(conf/10.0,2) end as confidence, context from (
