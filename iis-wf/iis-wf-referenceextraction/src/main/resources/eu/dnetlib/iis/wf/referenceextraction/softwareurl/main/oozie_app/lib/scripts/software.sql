...
when regexprmatches("gitorious",match) then "Gitorious"
when regexprmatches("debian",match) then "Debian"
when regexprmatches("gnu",match) then "GNU" else "Other"
end as repo
from
...
"(https*\:\/\/code\.google\.com\/p\/(\w+|\-))|(https*\:\/\/github.com\/.+\/(\w+|\-))|(https*\:\/\/bitbucket.org\/.+\/.+)|(https*\:\/\/sourceforge\.net\/projects\/.+)|(debian\.org)|(ftp\.gnu\.org)|(gitorious\.com)") from
