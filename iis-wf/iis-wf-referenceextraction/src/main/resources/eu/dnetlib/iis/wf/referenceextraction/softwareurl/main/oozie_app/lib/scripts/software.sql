...
(select id,match,repo,case when title is null then "" else title end as title,case when description is null then "" else description end as description,url from (select id, match, repo,
case
...
from (select distinct id,match,repo, case when data is null then "" else data end as data from (
select id,match,
case
