...
select jdict('documentId', docid, 'projectId', id, 'originalid',grantid,'context',context,'confidenceLevel', sqroot(min(1.49,confidence)/1.5)) from ( select docid,id,j2s(prevpack,middle,nextpack) as context,max(confidence) as confidence, fundingClass1, grantid from ( select docid, id, prevpack,middle,nextpack,fundingClass1, grantid,
