...
(regexpr("\b(\w{2,5}\d{2,4}\w{0,6}(?:-\w+)?(?: \w+)?)\b",middle)=grantid and fundingclass1 = "ICS") or
(regexpr("\b(ANR-\d{2}-\w{4}-\d{4})\b",middle)=grantid and fundingclass1 = "ANR") or
(regexpr("\b([A-Za-z_]+(?:-[A-Za-z_]+)?-20\d{2}-\d{2,})\b",middle)=grantid and fundingclass1 = "HRB" and not regexprmatches("zhengzhou",lower(j2s(prev,middle,next)))) or
