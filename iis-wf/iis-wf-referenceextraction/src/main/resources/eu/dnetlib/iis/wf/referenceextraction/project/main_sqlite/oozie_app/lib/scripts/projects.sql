...
select jdict('documentId', docid, 'projectId', id, 'confidenceLevel', 0.8, 'textsnippet', textsnippet_) as C1, docid, id from (
 select  docid, id, textsnippet_ from 
  (select docid,textwindow2s(textsnippet_,2,1,1,"(?:\b|fm17)"||proj_id||"\b") as textwin, proj_id, id from ( 
   select docid,t2.Project_id as proj_id, t2.id,  t1.textsnippet_   from
    ( select distinct  docid, textsnippet_, jsplitv(regexprfindall('(?:\b|fm17)(\d{2,4})\b',textsnippet_)) as res
      from( select docid, filterstopwords(lower(keywords(j2s(prev,middle,next)))) as textsnippet_ from  hfri_unidentified_only) ) as t1, 
...
delete from matched_undefined_hfri where docid in (select docid from output_hfri);
...
union all
select jdict('documentId', docid, 'projectId', id, 'confidenceLevel', 0.8, 'textsnippet', prev||" "||middle||" "||next) from matched_undefined_hfri;
