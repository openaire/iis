...
public class ResearchInitiativeReferenceExtractionInputTransformerJob {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (JavaSparkContext sc = JavaSparkContextFactory.withConfAndKryo(new SparkConf())) {
            ...
            JavaPairRDD<CharSequence, Tuple4<CharSequence, Integer, List<Author>, List<CharSequence>>> idToMeta = inputMeta
                    .mapToPair(x -> new Tuple2<>(x.getId(), new Tuple4<>(x.getTitle(), x.getYear(), x.getImportedAuthors(), x.getExtractedAuthorFullNames())));
            JavaPairRDD<CharSequence, CharSequence> idToText = inputText
                    .mapToPair(x -> new Tuple2<>(x.getId(), x.getText()));
            JavaPairRDD<CharSequence, Tuple2<Tuple4<CharSequence, Integer, List<Author>, List<CharSequence>>, Optional<CharSequence>>> joined = idToMeta
                    .leftOuterJoin(idToText);
