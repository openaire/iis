...
public class DocumentMetadataConverter extends AbstractAvroConverter<DocumentMetadata> {
     ...
     */
    public DocumentMetadataConverter(String encoding, ResultApprover resultApprover, FieldApprover fieldApprover,
            byte[] personResultAuthorshipHasAuthorColumnFamilyBytes) {
        super(resultApprover);
        this.encoding = encoding;
        this.fieldApprover = fieldApprover;
        this.personResultAuthorshipHasAuthorColumnFamilyBytes = OafHelper
                .copyArrayWhenNotNull(personResultAuthorshipHasAuthorColumnFamilyBytes);
    }

    @Override
    public DocumentMetadata buildObject(Result hbaseResult, Oaf resolvedOafObject) throws IOException {
        ResultProtos.Result sourceResult = resolvedOafObject.getEntity() != null
                ? resolvedOafObject.getEntity().getResult() : null;
        if (sourceResult == null) {
            log.error("skipping: no result object " + "for a row " + new String(hbaseResult.getRow(), encoding));
            return null;
        }
        if (resolvedOafObject.getEntity().getId() != null) {
            ...
            log.error(
                    "skipping: no id specified for " + "result of a row " + new String(hbaseResult.getRow(), encoding));
            return null;
        }
    }
    ...
    private DocumentMetadata.Builder createBasicMetadata(ResultProtos.Result sourceResult,
        ...
        if (sourceResult.getMetadata() != null) {
                    ...
                    for (StructuredProperty titleProp : sourceResult.getMetadata().getTitleList()) {
                        if (titleProp.getQualifier() != null
                                && HBaseConstants.SEMANTIC_CLASS_MAIN_TITLE
                                        .equals(titleProp.getQualifier().getClassid())
            ...
            if (sourceResult.getMetadata().getLanguage() != null
                    ...
                    && !LANG_CLASSID_UNDEFINED.equals(sourceResult.getMetadata().getLanguage().getClassid())) {
            ...
            if (sourceResult.getMetadata().getDateofacceptance() != null
                ...
                Integer yearValue = extractYear(sourceResult.getMetadata().getDateofacceptance().getValue());
                if (yearValue != null) {
            ...
            if (sourceResult.getMetadata().getSubjectList() != null
                ...
                Collection<String> extractedKeywords = extractValues(sourceResult.getMetadata().getSubjectList());
     ...
     * @throws InvalidProtocolBufferException
     */
    private DocumentMetadata.Builder handleAdditionalIds(Oaf resolvedOafObject, DocumentMetadata.Builder metaBuilder)
            throws InvalidProtocolBufferException {
        ...
        if (resolvedOafObject != null && resolvedOafObject.getEntity() != null
            ...
            for (StructuredProperty currentPid : resolvedOafObject.getEntity().getPidList()) {
                ...
                if (currentPid != null && currentPid.getQualifier() != null
                        && currentPid.getQualifier().getClassid() != null && currentPid.getValue() != null
     ...
     * @throws InvalidProtocolBufferException
     */
    private DocumentMetadata.Builder handleDatasourceIds(Oaf resolvedOafObject, DocumentMetadata.Builder metaBuilder)
            throws InvalidProtocolBufferException {
        if (resolvedOafObject != null && resolvedOafObject.getEntity() != null) {
     ...
     * @throws IOException
     */
    private DocumentMetadata.Builder handlePersons(Result hbaseResult, DocumentMetadata.Builder builder)
            throws IOException {
        NavigableMap<byte[], byte[]> personResultRelations = hbaseResult
                .getFamilyMap(personResultAuthorshipHasAuthorColumnFamilyBytes);
        ...
        for (byte[] personResultBytes : personResultRelations.values()) {
            ...
            if (resultApprover != null ? resultApprover.approveBeforeBuilding(persResOAF) : true) {
     ...
     * element whether this piece of information was not generated by IIS.
     * 
     * @param source
