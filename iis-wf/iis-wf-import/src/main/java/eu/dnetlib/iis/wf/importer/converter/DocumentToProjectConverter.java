...
public class DocumentToProjectConverter extends AbstractAvroConverter<DocumentToProject[]> {
     ...
     */
    public DocumentToProjectConverter(ResultApprover resultApprover,
            byte[] resultProjectOutcomeIsProducedByColumnFamilyBytes) {
        super(resultApprover);
        this.resultProjectOutcomeIsProducedByColumnFamilyBytes = OafHelper
                .copyArrayWhenNotNull(resultProjectOutcomeIsProducedByColumnFamilyBytes);
    }

    @Override
    public DocumentToProject[] buildObject(Result hbaseResult, Oaf resolvedOafObject)
            throws InvalidProtocolBufferException {
        NavigableMap<byte[], byte[]> resProjRelations = hbaseResult
                .getFamilyMap(resultProjectOutcomeIsProducedByColumnFamilyBytes);
        if (resProjRelations != null && resProjRelations.size() > 0) {
            List<DocumentToProject> results = new ArrayList<DocumentToProject>(resProjRelations.size());
            for (byte[] resProjBytes : resProjRelations.values()) {
                ...
                if (resultApprover != null ? resultApprover.approveBeforeBuilding(resultProjOAF) : true) {
