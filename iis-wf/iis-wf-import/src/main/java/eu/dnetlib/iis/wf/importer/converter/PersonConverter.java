...
public class PersonConverter extends AbstractAvroConverter<Person> {
     ...
     */
    public PersonConverter(String encoding, ResultApprover resultApprover) {
    ...
    public Person buildObject(Result source, Oaf resolvedOafObject) throws UnsupportedEncodingException {
        ...
        if (sourcePerson == null) {
            log.error("skipping: no person object " + "for a row " + new String(source.getRow(), encoding));
            return null;
        }
        if (resolvedOafObject.getEntity().getId() != null) {
            if (sourcePerson.getMetadata() != null) {
                ...
                log.error("skipping: no metadata specified for " + "person of a row "
                        + new String(source.getRow(), encoding));
                return null;
            }
        } else {
            log.error("skipping: no id specified for " + "person of a row " + new String(source.getRow(), encoding));
            return null;
        }
    }
