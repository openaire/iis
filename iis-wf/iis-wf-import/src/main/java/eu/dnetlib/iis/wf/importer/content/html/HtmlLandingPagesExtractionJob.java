...
import eu.dnetlib.iis.metadataextraction.schemas.DocumentTextWithChecksum;
 ...
 * Provides {@link DocumentTextWithChecksum} avro records at the output.
 * 
 * @author mhorst
 */
public class HtmlLandingPagesExtractionJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (SparkSession session = SparkSessionFactory.withConfAndKryo(new SparkConf())) {
            ...
            JavaRDD<DocumentTextWithChecksum> output = grouped.flatMap(tuple -> {
                ...
                for (Row r : rows) {
                    ...
                    htmlMap.computeIfAbsent(htmlFilename, k -> new ArrayList<>())
                           .add(new Tuple2<>(id, htmlHash));
                }

                // Download and extract archive from S3
                List<DocumentTextWithChecksum> records = new ArrayList<>();
                    ...
                    while ((entry = tarInput.getNextTarEntry()) != null) {
                        ...
                        if (htmlMap.containsKey(entry.getName())) {
                            ...
                            for (Tuple2<String, String> pair : htmlMap.get(entry.getName())) {
                                DocumentTextWithChecksum.Builder documentTextBuilder = DocumentTextWithChecksum.newBuilder();
                                documentTextBuilder.setId(pair._1());
                                documentTextBuilder.setChecksum(pair._2());
    ...
    private static void storeInOutput(JavaRDD<DocumentTextWithChecksum> results, JavaRDD<ReportEntry> reports,
            String resultOutputPath, String reportOutputPath) {
        avroSaver.saveJavaRDD(reports, ReportEntry.SCHEMA$, reportOutputPath);
        avroSaver.saveJavaRDD(results, DocumentTextWithChecksum.SCHEMA$, resultOutputPath);
    }
