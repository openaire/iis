...
public class DeduplicationMappingConverter extends AbstractAvroConverter<IdentifierMapping[]> {
     ...
     */
    public DeduplicationMappingConverter(ResultApprover resultApprover,
            byte[] resultResultDedupMergesColumnFamilyBytes) {
        super(resultApprover);
        this.resultResultDedupMergesColumnFamilyBytes = OafHelper
                .copyArrayWhenNotNull(resultResultDedupMergesColumnFamilyBytes);
    }

    @Override
    public IdentifierMapping[] buildObject(Result hbaseResult, Oaf resolvedOafObject)
            throws InvalidProtocolBufferException {
        NavigableMap<byte[], byte[]> dedupRelations = hbaseResult
                .getFamilyMap(this.resultResultDedupMergesColumnFamilyBytes);
        if (dedupRelations != null && dedupRelations.size() > 0) {
            List<IdentifierMapping> results = new ArrayList<IdentifierMapping>(dedupRelations.size());
            for (byte[] resultResultBytes : dedupRelations.values()) {
                ...
                if (resultApprover != null ? resultApprover.approveBeforeBuilding(resResOAF) : true) {
